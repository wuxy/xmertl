...
static int prtk_shm_ioctl(int fd, unsigned int cmd, unsigned long arg)
{
	...
	dev = ((XM_SHM_DEV << 16) & 0xFFFF0000) | (prtk_shm.pshm[minor].shm_xmminor & 0x0000FFFF);
	...
	int prot, int flags, int offset, char **result)
{
 	void * nstart;
	int minor = MINOR(fd);
	unsigned long dev = ((XM_SHM_DEV << 16) & 0xFFFF0000) | (prtk_shm.pshm[minor].shm_xmminor & 0x0000FFFF);
	int err = 0;
...
static int prtk_shm_open(char *name, int mode)
{
	...
	prtk_shm.pshm[minor].shm_xmminor = xm_shm_open(nname, 0, 0);
