...
#define ERROR_MSG(fmt, args...) printk(fmt, ## args)
...
void *realloc_ex(void *ptr, size_t new_size, void *mem_pool)
{
    ...
    tmp_size = (b->size & TLSF_BLOCK_SIZE);
    if (new_size <= tmp_size) {
        tlsf->used_size -= (b->size & TLSF_BLOCK_SIZE) + BHDR_OVERHEAD;
        if (next_b->size & FREE_BLOCK) {
            MAPPING_INSERT(next_b->size & TLSF_BLOCK_SIZE, &fl, &sl);
            EXTRACT_BLOCK(next_b, tlsf, fl, sl);
            tmp_size += (next_b->size & TLSF_BLOCK_SIZE) + BHDR_OVERHEAD;
    	    next_b = GET_NEXT_BLOCK(next_b->ptr.buffer,
				    next_b->size & TLSF_BLOCK_SIZE);
    ...
    if ((next_b->size & FREE_BLOCK)) {
      if (new_size <= (tmp_size + (next_b->size & TLSF_BLOCK_SIZE))) {
        tlsf->used_size -= (b->size & TLSF_BLOCK_SIZE) + BHDR_OVERHEAD;
	MAPPING_INSERT(next_b->size & TLSF_BLOCK_SIZE, &fl, &sl);
        EXTRACT_BLOCK(next_b, tlsf, fl, sl);
	b->size += (next_b->size & TLSF_BLOCK_SIZE) + BHDR_OVERHEAD;
        next_b = GET_NEXT_BLOCK(b->ptr.buffer, b->size & TLSF_BLOCK_SIZE);
	next_b->prev_hdr = b;
	next_b->size &= ~PREV_FREE;
        tmp_size = (b->size & TLSF_BLOCK_SIZE) - new_size;
        if (tmp_size >= sizeof (bhdr_t)) {
	    tmp_size -= BHDR_OVERHEAD;
            tmp_b = GET_NEXT_BLOCK(b->ptr.buffer, new_size);
            tmp_b->size = tmp_size | FREE_BLOCK | PREV_USED;
    ...
    cpsize = ((b->size & TLSF_BLOCK_SIZE) > new_size) ?
	new_size : (b->size & TLSF_BLOCK_SIZE);
...
EXPORT_SYMBOL(malloc_ex);
EXPORT_SYMBOL(free_ex);
EXPORT_SYMBOL(init_memory_pool);
